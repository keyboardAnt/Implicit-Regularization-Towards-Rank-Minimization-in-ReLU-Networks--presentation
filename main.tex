...
\begin{frame}{Implicit bias in linear~networks: Matrix~fact.}
    ...
    \begin{block}{\cite{razin2020implicit}’s conjecture}
        (1) The implicit~bias in \emph{matrix~fact.} can be characterized by \true{rank~minimization}. (2) Some notion of rank~min. may be key to explaining generalization in deep~learning.
    \end{block}
    \pause
    ...
    % % \begin{itemize}
    % %     % \item \cite{li2020towards}’s evidence:\\
    % %     % \emph{Matrix~fact.} is implicitly biased towards rank~min.
    % %     \pause
    % %     \item \cite{razin2021implicit}’s theoretical \& empirical results:\\
    % %     \emph{Tensor~fact.} (a generalization) is implicitly biased towards low-rank~tensors.
    % \end{itemize}
