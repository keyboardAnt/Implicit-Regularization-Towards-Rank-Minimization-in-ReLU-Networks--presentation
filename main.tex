...
\begin{frame}{Preliminaries}
    ...
    \onslide<7->{
        \begin{description}
            \item[overparameterized neural network] $N_\btheta$, where $\sum_{\ell = 1}^k d_\ell \cdot d_{\ell-1} >> n$.
        \end{description}
    }
...
\begin{frame}
    \begin{columns}
            \begin{column}{0.5\textwidth}
                \begin{figure}[t]
                    \centering
                    \frame{\includegraphics[scale=.24]{figures/D_and_Ss_regions.png}}
            ...
            \begin{column}{0.7\textwidth}
                ...
                \begin{eqnarray*}
                    &\mathcal{D} := \{ \bw \mid \forall i \in [2], \bw^\top \bx_i \le 0\}\\
                    \pause
                    &\mathcal{S} := \{ \bw \mid \forall i \in [2], \bw^\top \bx_i > 0 \}\\
                    \pause
                    &\mathcal{S}_j := \{ \bw  \mid  i=j \Leftrightarrow \bw^\top \bx_i > 0 \}
                \end{eqnarray*}
                }
            \end{column}
    \end{columns}
    ...
    \note{
        ...
        \begin{enumerate}
            \item Intuitively, $\cd$~defines the ``dead''~region where neurons output~$0$ on both~$\bx_1,\bx_2$ after applying the ReLU. In other words, any neuron in~$\cd$ has a non-positive correlation with both inputs. 
            \item $\cs$~is the ``active''~region where neurons have a positive~correlation with both~$\bx_1,\bx_2$.
            \item $\cs_1,\cs_2$~are the ``partially~active''~regions, where neurons are active for only the corresponding~input. For~example, each neuron in~$\cs_1$ has a positive~correlation with~$\bx_1$ and non-positive~correlation with~$\bx_2$.
        \end{enumerate}
    }
\end{frame}
...
\begin{frame}{Our results}{Rank~min. in deep~networks with small~$\ell_2$~norm}
    ...
    \note{
        \footnotesize
        For ReLU~networks that are overparameterized in~terms of depth and have width~$\ge 2$, we identify settings in which they \textbf{are} biased towards low~ranks.
        \begin{enumerate}
            \itemsep=0pt
            \topsep=0pt
            \item First, we consider ReLU~networks trained w.r.t. the square~loss.
            \item We show that, in sufficiently~deep~networks that attain $0$~loss and minimize the $\ell_2$-norm~of~the~parameters, the average~ratio between the spectral and the Frobenius~norms of the weight~matrices is close~to~$1$.
