...
\begin{frame}
    ...
    with Gal Vardi and Ohad Shamir.
\end{frame}
...
\begin{frame}{What is \emph{implicit regularization}?}{Generalization despite overparameterization}
    ...
    \pause
    \begin{itemize}
        \item \alert{Many} global minima with $0$ training loss.
    \end{itemize}
    \pause
    \begin{block}{Phenomenon in practice (\cite{zhang2017understanding})}
        GD over such overparameterized neural networks generalize well even when trained \true{without~explicit~regularization}.
    \end{block}
    \textbf{Q:} How?\\
    \pause
...
\begin{frame}{Previous work}{Implicit regularization in *nonlinear* networks with square~loss}
    \pause
...
\begin{frame}{Our results}{GF does~not even approximately minimize ranks}
    \pause
    Setting: \true{ReLU}~networks of depth $2$ and width $2$ with multiple outputs, trained with the square~loss.
    \pause
...
\AtBeginSection[]
{
    ...
    \note{
        \footnotesize
        The last part of our work is about ReLU~networks in binary~classification~problems.
    }
}
...
\begin{frame}{Previous work}{Implicit bias in linear~networks with exponentially-tailed~losses}
    ...
    \note{
        ...
        \begin{enumerate}
            ...
            \item We say that a~trajectory~$\btheta(t)$~of~GF {\em converges~in~direction}~to~$\tilde{\btheta}$ if, when~$t \to \infty$, $\tilde{\btheta}$ is the limit of the~direction~of~$\btheta(t)$.
        \end{enumerate}
    }
\end{frame}
