\documentclass[usenames,dvipsnames]{beamer} % To enable \pause: remove the "handout" preamble
\usetheme{Boadilla}
...
\begin{frame}
    ...
    \note{
        \begin{enumerate}
            ...
            \itemsep=0pt
            \topsep=0pt
            \item We have two $2$-by-$2$ weight matrices, $W$ and $V$, that parameterize a (depth-$2$~width-$2$)~network~$N_{W,V}$. We assume that the network induces $0$~loss after training via GF w.r.t. the square~loss over the given training~dataset of size~$2$, $(X, Y)$.
            ...
            \item The main idea is to construct a rank-deficient network, where its two neurons in the first layer, $w_1, w_2$ have precisely opposite directions s.t. each neuron is active for exactly one input. 
...
\begin{frame}
    ...
    \note{
        \begin{enumerate}
            ...
            \item Second, an obtuse angle between the two input vectors.
            \item For technical convenience, we also assume that both $\bx_i$ and $\by_i$ have the unit length, but we believe it is not essential.
...
\begin{frame}
    ...
    \note{
        The following theorem shows that for any given dataset satisfying the two said assumptions and small enough initialization, with at~least constant probability, GF converges to a zero-loss~solution that is not even close to a low-rank~solution. In particular, it guarantees that the norms of the row vectors $\bw_i$ are bounded away from~$0$ s.t. the ratio between the norms is bounded. Moreover, the angle between $\bw_1$ and $\bw_2$ is bounded away from~$0$ and from~$\pi$. All those bounds consist of explicit large constants that depend only on the dataset.
        \newline
        \newline
        The conclusion is that, with at least constant probability, GF does~not minimize any reasonable approximate notion of rank.
        \newline
        \newline
...
\begin{frame}{GF does~not even approximately minimize~ranks}{An empirical result}
    ...
    \note{
        We saw that, for small enough initialization schemes, GF does not converge close~to low-rank~solutions, with some positive~probability. We now present a simple experiment that corroborates this and suggests that, furthermore, this holds with~high~probability and for standard initializations.
        \begin{enumerate}
            \itemsep=0pt
            \topsep=0pt
            \item We consider the ``stable/numerical~rank" that approximates the standard~algebraic~rank. Intuitively, it measures the concentration of the singular values in the first singular value. Its image ranges between $1$ and the maximal standard~rank, which equals $2$ in our case.
            \item The figure is a histogram of the stable ranks of the two weight~matrices at convergence, $W1$ and $W2$, based on a few hundred repeats with a unique initialization. It counts the number of runs that converged to some stable rank. 
...
\begin{frame}{Proof ideas}
    ...
    \note{
        Now, to the main ideas in the proofs. We start with the this theorem. I~will briefly remind the statement.
        \begin{enumerate}
...
\begin{frame}
    ...
    \note{
        The figure depicts the 2D plain and the given input vectors $\bx_1, \bx_2$ on the unit sphere. Intuitively, $\cd$~defines the ``dead''~region where neurons output~$0$ on both~$\bx_1,\bx_2$. $\cs$~is the ``active''~region where neurons have a positive~output on both~$\bx_1,\bx_2$. $\cs_1,\cs_2$~are the ``partially~active''~regions, where neurons are active for only the corresponding input. It means, for example, that each neuron in $\cs_1$ is positively correlated with $\bx_1$ and non-positively correlated with $\bx_2$.
    }
\end{frame}
...
\begin{frame}
    ...
    \note{
        \begin{enumerate}
            \itemsep=0pt
            \topsep=0pt
            \item Assume towards~contradiction that GF converges to some $0$-loss~network~$N_{W(\infty),V(\infty)}$ where~$\rank(W(\infty))<2$. 
            \item The $0$-loss assumption means that $Y = V(\infty) \sigma \left( W(\infty) X \right)$.
            \item Hence, the rank of both sides equals $2$. It implies that the first layer's output matrix, $\sigma(W(\infty)X)$ is full-ranked.
            \item Therefore, the weight~vectors~$\bw_1(\infty)$ and~$\bw_2(\infty)$ are~not in region~$\cd$. Indeed, if $\bw_1(\infty)$ or~$\bw_2(\infty)$ are in~$\cd$, then at~least one of the rows of~$\sigma(W(\infty) X)$ is~$\zero$, in~contradiction.
            ...
            \item Note that if $\alpha$ was~positive, we could move it outside the $\sigma$, then we have the two rows of the first layer's output matrix, $\sigma(W(\infty)X)$ linearly dependent, contradicting what we showed (i.e., that it is full-ranked). Therefore, $\alpha$ is strictly negative.
            \item Imagine the line between $\bw_1(\infty), \bw_2(\infty)$. It must go through the origin. Since we also have~$\bw_1(\infty),\bw_2(\infty) \not \in \cd$, then one of these weight~vectors is in interior~of~$\cs_1$ and the other is in interior~of~$\cs_2$. W.l.o.g. $\bw_j(\infty) \in \cs_j$'s interior.
        \end{enumerate}
    }
\end{frame}
...
\begin{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            ...
            \pause
            \hline \newline
            $\bw_i(0) \sim$ a spherically~symmetric distribution.
            \begin{itemize}[<+->]
    ...
    \note{
        The key observation in the proof is analyzing the dynamics where $\bw_i(\infty) \in \cs_i$'s interior for all $i$. Suppose that, indeed, $\bw_1(0) \in \cs_1 \setminus \partial \cs_1$ and~$\bw_2(0) \in \cs_2 \setminus \partial \cs_2$.
        \begin{enumerate}
            \item Then, it is possible to show that the dynamics are such that $\bw_i$ moves towards $\bx_i$ until convergence and never leaves the interior of $\cs_i$.
