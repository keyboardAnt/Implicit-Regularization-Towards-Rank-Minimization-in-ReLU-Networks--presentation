...
\begin{frame}{Our results}{GF doesn't even approximately minimize ranks}
    ...
    \begin{exampleblock}{\cite{li2020towards} (matrix~fact.)}
    ...
    \begin{exampleblock}{Thm. (informal) \hfill\checkmark}
          GF doesn't converge to a low-rank~solution for size-$2$ datasets, $\{(\bx_1,\by_1),(\bx_2,\by_2)\} \subseteq \mathbb{S}^1 \times \mathbb{S}^1$, whenever $\measuredangle(\bx_1,\bx_2) \in (\pi/2,\pi)$ and $\by_1,\by_2$ are linearly~independent.
    \end{exampleblock}
    \pause
    \begin{itemize}
