...
\begin{frame}{GF does~not even approximately minimize~ranks}{An empirical result}
    ...
    \note{
        We saw that, for reasonable initialization schemes, GF does not converge close~to low-rank~solutions, with some positive~probability. We now present a simple experiment that corroborates this and suggests that, furthermore, this holds with~high~probability and for standard initializations.
        \begin{enumerate}
            \itemsep=0pt
            \topsep=0pt
            ...
            \item The figure is a histogram of the stable ranks of the two weight~matrices at convergence, $W1$ and $W2$, based on a few hundred repeats. It counts the number of runs that converged to some stable rank. 
