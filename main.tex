...
\begin{frame}
    ...
    \begin{enumerate}
        ...
        \item Note that the result does~not depend on the width of the networks. Even if train an extremely~wide network, the bias towards low~ranks exists as~long~as its depth,~$k'$, is~large. Also, the theorem holds even if the network~$N$ have high~ranks (e.g., all its weight~matrices have full rank).
    \end{enumerate}
    }
\end{frame}
