...
\begin{frame}{Our results}{GF does~not even approximately minimize ranks}
    \pause
    Settings: \true{ReLU}~networks of depth $2$ and width $2$ w/ multiple outputs,\\
    \pause
    ...
    \begin{exampleblock}{Thm. (informal) \hfill\checkmark}
          GF does~not converge to a low-rank~solution for size-$2$ datasets, $\{(\bx_1,\by_1),(\bx_2,\by_2)\} \subseteq \mathbb{S}^1 \times \mathbb{S}^1$, whenever $\measuredangle(\bx_1,\bx_2) \in (\pi/2,\pi)$ and $\by_1,\by_2$ are linearly~independent.
    \end{exampleblock}
    \pause
    \begin{itemize}
    ...
    \pause
    \begin{exampleblock}{Thm. (informal) \hfill\checkmark}
          The solutions that GF converges to are not even close to have low~rank, w.p. at least constant.
    \end{exampleblock}
\end{frame}
...
\begin{frame}
    \pause
    \begin{itemize}
        \item Let $(X, Y) \in \mathbb{R}^{2 \times 2} \times \mathbb{R}^{2 \times 2}$ (a training dataset), and $W,V\in \reals^{2 \times 2}$ s.t. $N_{W,V}(X)=V\sigma(WX)$ (weight~matrices of a 0-loss~solution).
        \pause
    ...
    \pause
    \begin{itemize}
        \item Construct (the weight~vectors of) the neurons in the $1$st~layer to have opposite directions (hence the weight~matrix has rank $1$), s.t. each neuron is active for exactly one input.
        \pause
    ...
    \note{
        \begin{enumerate}
            \item "$\exists$ such a low-rank~0-loss~solution?"\\
            Otherwise, this setting is trivial.
        \end{enumerate}
    }
\end{frame}
